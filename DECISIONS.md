...
## Q158 — interactive/schema-drift — gate-resolution

**Question:** `schema_drift.rs` is a tripwire on a refresh: it fires when somebody re-vendors `docs/research/tailscale-openapi.yaml`, and asks Tailscale nothing. Nothing in a push changes that file, and a finished project never re-vendors, so the check that guards the one input guaranteed to rot cannot fire. Give it a clock — and if so, fetching from where, and failing how?
**Options considered:** leave it (a refresh is a human act) / a scheduled reminder to re-vendor by hand / a scheduled conditional GET that fails on drift / the same, opening an issue instead of failing
**Chosen:** A weekly conditional GET against the address the vendored copy came from, failing the run on drift.
**Decided-by:** agent
