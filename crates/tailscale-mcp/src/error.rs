...
impl ToolError {
    ...
    pub fn conflict(message: impl Into<String>) -> Self {
        Self::new(ErrorCode::Conflict, message).with_hint(
    ...
    pub fn result_too_large(bytes: usize, cap: usize) -> Self {
        ...
        .with_hint("Narrow the request, or raise TAILSCALE_MCP_MAX_RESULT_BYTES on the server.")
    }
...
mod tests {
    ...
    fn key_shaped_values_are_removed_from_every_field() {
        let err = ToolError::cli_failed(
            "tailscale up",
            Some(1),
            "invalid key: tskey-auth-kXaMpLe1CNTRL-secretpart",
        );
    ...
    fn each_key_shape_is_recognised() {
        for input in [
            "tskey-auth-abc123-def456",
            "tskey-api-abc123-def456",
            "tskey-client-abc123-def456",
            "tskey-abc123def456",
        ] {
    ...
    fn bearer_tokens_are_removed() {
        let out = redact("Authorization: Bearer tskey-api-abc-def");
        assert_eq!(out, "Authorization: Bearer [redacted]");
    }

    #[test]
    fn several_secrets_in_one_string_are_all_removed() {
        let out = redact("old tskey-auth-a1-b2 new tskey-auth-c3-d4 done");
        assert_eq!(
            out,
    ...
    fn the_redactor_also_scrubs_values_it_was_given() {
        ...
        let out = r.apply("failed with an-oauth-client-secret-value and tskey-api-a-b");
        assert_eq!(
            out,
