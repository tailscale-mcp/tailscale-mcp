...
//! accepting an invite act as a person: the invite records who sent it, and an
...
crate::tools! {
    ...
    tailnet_device_invite_get => DeviceInviteParams, device_invite_get,
        toolset: TailnetInvites, tier: Read, idempotent: true;

    /// Withdraw a device invitation. Anyone who already accepted keeps the
    /// share; this only stops it being accepted again.
    tailnet_device_invite_delete => DeviceInviteParams, device_invite_delete,
    ...
    tailnet_device_invite_resend => DeviceInviteParams, device_invite_resend,
        toolset: TailnetInvites, tier: Write, idempotent: true;
    ...
    tailnet_user_invite_get => UserInviteParams, user_invite_get,
        toolset: TailnetInvites, tier: Read, idempotent: true;

    /// Withdraw a tailnet invitation. Needs a user-owned credential.
    tailnet_user_invite_delete => UserInviteParams, user_invite_delete,
        toolset: TailnetInvites, tier: Destructive, idempotent: true;
    ...
    /// Only for an invite that was created with an email address, and rate
    /// limited upstream to one a minute. Needs a user-owned credential.
    tailnet_user_invite_resend => UserInviteParams, user_invite_resend,
        toolset: TailnetInvites, tier: Write, idempotent: true;
}
...
/// `multiUse` is Tailscale's spelling and `multi_use` is ours, so the rename
/// has to be written out; the derive would send `multi_use`.
impl DeviceInviteRequest {
    fn to_body(&self) -> Value {
        let mut body = serde_json::Map::new();
        if let Some(multi_use) = self.multi_use {
            body.insert("multiUse".to_owned(), Value::Bool(multi_use));
        }
        if let Some(allow) = self.allow_exit_node {
            body.insert("allowExitNode".to_owned(), Value::Bool(allow));
        }
...
async fn device_invite_create(
    ...
    let body: Vec<Value> = params
        .invites
        .iter()
        .map(DeviceInviteRequest::to_body)
        .collect();
    client
        ...
        .await
        .map(as_listing)
        .map_err(|error| explain_credential(ToolError::from(error)))
}

async fn device_invite_get(ctx: &ToolContext, params: DeviceInviteParams) -> ToolResult<Value> {
    let client = ctx.tailnet()?;
    Ok(client
...
async fn device_invite_delete(ctx: &ToolContext, params: DeviceInviteParams) -> ToolResult<Value> {
    let client = ctx.tailnet()?;
    client
...
async fn device_invite_resend(ctx: &ToolContext, params: DeviceInviteParams) -> ToolResult<Value> {
    let client = ctx.tailnet()?;
...
async fn user_invite_create(
    ...
    let body: Vec<Value> = params
        ...
        .map(|invite| {
            ...
            entry.insert(
                "role".to_owned(),
                Value::String(one_of("role", &invite.role, INVITE_ROLES)?),
            );
...
async fn user_invite_get(ctx: &ToolContext, params: UserInviteParams) -> ToolResult<Value> {
    let client = ctx.tailnet()?;
    Ok(client
...
async fn user_invite_delete(ctx: &ToolContext, params: UserInviteParams) -> ToolResult<Value> {
    let client = ctx.tailnet()?;
    client
...
async fn user_invite_resend(ctx: &ToolContext, params: UserInviteParams) -> ToolResult<Value> {
    let client = ctx.tailnet()?;
