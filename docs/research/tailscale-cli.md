...
- `lock init` without `--confirm` prints its warning, reads end-of-file from a closed standard input and **exits 0 having initialised nothing** — a silent no-op reported as success. Probed on 1.102.2 against a tailnet with lock off, which stayed off. Any non-interactive caller has to pass `--confirm=true` (DECISIONS Q39).
- `lock sign` accepts `file:<path>` for the node key and the rotation key; `lock disable` and `lock disablement-kdf` do not, and take their secret as a bare positional (DECISIONS Q38).
- The two disablement commands want the same secret in different shapes: `lock disable` refuses anything without a `disablement:` or `disablement-secret:` prefix (`parsing argument 1: expected value with "disablement:" or "disablement-secret:" prefix`), while `lock disablement-kdf` wants bare hex and fails a prefixed value at the hex decoder (`encoding/hex: invalid byte: U+0069 'i'`). It prints the public `disablement:<hex>` value on stdout and exits 0.
- `lock revoke-keys` has two usage forms behind one positional — keys to revoke, or the recovery blob from the previous step — and tells them apart by parsing, so `revoke-keys --cosign --finish tlpub:…` fails with `parsing hex: encoding/hex: invalid byte: U+0074 't'` rather than with anything about the flags (DECISIONS Q42).
- Tailnet-lock failures on a tailnet with lock off are worded three ways and none of them classifies as `not_found`: `tailnet lock is not enabled` from `add`/`remove`, `error: 400 Bad Request: tailnet-lock disable failed: tailnet-lock is not active` from `disable` and `local-disable`, and `generation of recovery AUM failed: sending generate-recovery-aum: 500 Internal Server Error: tailnet-lock is not active` from `revoke-keys`. `lock add` on a malformed key says `parsing key 1: key hex string doesn't have expected type prefix tlpub:`.
